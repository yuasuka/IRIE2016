...
pairA=[]
pairB=[]
for p in tqdm(testList[1:], desc="Model 執行..."):
    #print(p)
    ID = p[0]
    idList.append(ID)
    pairs = p[1:3] 
    ...
    pairB.append(p[2])
    LocalCorpus = []
    tempKeyA = pseg.lcut(p[1])
    tempKeyB = pseg.lcut(p[2])
    for doc in corpus:
                ...
                for W in Dictionary[R]:
                    for w in W:
                        ...
                        if key[0]==w[0]:
                            #print("Match")
                            scorelist[R] += w[1]#*w[1]
        if list(tempKeyB[0])[1] in ['nr']:
            for r in ['birthPlace', 'deathPlace', 'workPlace']:#['parent', 'child', 'spouse', 'sibling']:
                scorelist[r]=scorelist[r]*0.5
        elif list(tempKeyB[0])[1] in ['ns','t']:
            for r in ['parent', 'child', 'spouse', 'sibling']:#['birthPlace', 'deathPlace', 'workPlace']:
                scorelist[r]=scorelist[r]*0.5
        if max(scorelist, key=lambda i: scorelist[i])!=0:
            EstimateResult =max(scorelist, key=(lambda key:scorelist[key]))
            relation.append(EstimateResult)
        else:
            if list(tempKeyB[0])[1] == 'nr':
                if list(pairs[0])[0]==list(pairs[1])[0]:
                    if len(pairs[0])==len(pairs[1]):
                        relation.append('sibling')
                    else:
                        if len(Dictionary['parent'])>len(Dictionary['child']):
                            relation.append('parent')
                        else:
                            relation.append('child')
                else:
                    relation.append('spouse')
            elif list(tempKeyB[0])[1] == 't' or list(tempKeyB[0])[1] == 'ns':
                relation.append('birthPlace')
            else:
                relation.append('others')
    else:
        if list(tempKeyB[0])[1] == 'nr':
            if list(pairs[0])[0]==list(pairs[1])[0]:
                if len(pairs[0])==len(pairs[1]):
...
with open("/Users/chenyu/Dropbox/IR2016F/ir_project2_data/1231answer2.csv", 'w', encoding=('utf8')) as f:
    writer1= csv.DictWriter(f, fieldnames = ["Id","Property"])
    writer1.writeheader()
    writer = csv.writer(f, delimiter=',')
    writer.writerows(zip(idList,relation))
