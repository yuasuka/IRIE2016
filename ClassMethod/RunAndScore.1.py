            ...
            for R in Dictionary.keys():#按順序一個一個字典查（之後可以考慮先按詞性分再查）
                for W in Dictionary[R]:
                    for w in W:
                        ...
                        if key[0]==w[0]:
                            #print("Match")
                            scorelist[R] += key[1]#*w[1]
        tempKey = nlp.tag(p[2],oov_level=4)
        if ['nr'] in tempKey[0]['tag']:
            for r in ['parent', 'child', 'spouse', 'sibling']:
                scorelist[r]=scorelist[r]*2
        elif ['ns'] in tempKey[0]['tag']:
...
with open("/Users/chenyu/Dropbox/IR2016F/ir_project2_data/1226answer3(boson).csv", 'w') as f:
